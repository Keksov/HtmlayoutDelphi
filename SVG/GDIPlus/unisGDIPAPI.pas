...
{  TGPColor = class
  ...
  function TColorToARGB(Color: Integer): ARGB;
...
var
  DoGDIPlusInitialization: Boolean = True;
...
begin
  DC := GetDC(0);
  try
    LastResult := GdipCreateFromHDC(DC, Graphics);
  finally
    ReleaseDC(0, DC);
  end;
...
begin
  ...
  if not DoGDIPlusInitialization then
    Exit;

  if GDIPlusAvailable then
  begin
    DoGDIPlusInitialization := False;
    Exit;
  end;
