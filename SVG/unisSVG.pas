...
uses
  ...
  MSXML2_TLB_Light,
  WideStringList,
  unisGDIPOBJ, unisGDIPAPI, unisGDIPOBJ2, unisGDIPKerning, unisGDIPPathText,
  unisSVGTypes,
  unisMatrix,
  unisSVGStyle;
...
//    function TransformComplete(X, Y: TFloat): TFPoint; overload;
  ...
  private
    ...
    procedure ReadChildren(const aNode: IXMLDOMNode); virtual;
  ...
  private
    ...
    procedure   Paint(const Graphics: TGPGraphics; Rects: PRectArray; RectCount: Integer);
...
public
    ...
    function    SaveToNode(const Parent: IXMLDOMNode; Left, Top, Width, Height: TFloat): IXMLDOMNode;

    procedure   SetBounds(const Bounds: TGPRectF);
    procedure   Scale(DX: TFloat; DY: TFloat = -1);
    procedure   PaintTo(DC: HDC; Bounds: TGPRectF; Rects: PRectArray; RectCount: Integer); overload;
    procedure   PaintTo(DC: HDC; Bounds: TRECT ); overload;
    procedure   PaintTo(MetaFile: TGPMetaFile; Bounds: TGPRectF; Rects: PRectArray; RectCount: Integer); overload;
    procedure   PaintTo(Graphics: TGPGraphics; Bounds: TGPRectF; Rects: PRectArray; RectCount: Integer); overload;
    procedure   PaintTo(Bitmap: TBitmap; Bounds: TGPRectF; Rects: PRectArray; RectCount: Integer); overload;
...
uses
  SysUtils,
  XMLHelp,
  unisGDIPUtils, unisSVGParse, unisSVGProperties, unisSVGColor, unisSVGPaint, unisSVGPath, unisSVGCommon;
...
procedure TSVGBasic.ReadChildren(const aNode: IXMLDOMNode);
var
  C: Integer;
  SVG: TSVGObject;
  Root: TSVG;
...
begin
  for C := 0 to aNode.childNodes.length - 1 do
  begin
    SVG := nil;

    node := aNode.childNodes[C];
...
procedure TSVG.PaintTo(DC: HDC; Bounds: TRECT );
begin
    PaintTo( DC, MakeRectF( @Bounds ), nil, 0 );
end;

procedure TSVG.PaintTo(DC: HDC; Bounds: TGPRectF; Rects: PRectArray; RectCount: Integer);
var
...
begin
  ...
  try
    ...
    //Graphics.SetRenderingOrigin( floor( Bounds.x ), floor( Bounds.y ) );
...
procedure TSVG.PaintTo(Graphics: TGPGraphics; Bounds: TGPRectF; Rects: PRectArray; RectCount: Integer);
var
  M: TGPMatrix;
  MA: TMatrixArray;
begin
