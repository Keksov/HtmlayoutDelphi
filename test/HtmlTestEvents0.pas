...
var
    ...
    elRoot   : THtmlHElement;
    elBody   : THtmlHElement;
    elEvents : THtmlHElement;
    elTest   : THtmlHElement;

{*******************************************************************************
* EventProxy
*******************************************************************************}
function EventProxy( tag : Pointer; he : HELEMENT; evtg : UINT; prms : Pointer ) : BOOL stdcall;
var
    eventGroup  : string;
    msg         : string;
    cmd         : HTMLayoutPhase;
...
begin
    ...
    HANDLE_BEHAVIOR_EVENT :
        begin
            ...
            cmd := THTMLayoutEvent.phase( PHTMLayoutBehaviorEventParams( prms ).cmd );
        end;
    HANDLE_METHOD_CALL :
        begin
    ...
    elEvents.innerHtml := eventGroup + msg + '<br/>' + elEvents.innerHtml;
end;
...
//    elTmp := nil;
    ...
    elRoot := THtmlHElement.Create( html.root );
    assert( elRoot.index = 0 );
    assert( elRoot.innerHtml <> '' );
    assert( elRoot.childrenCount = 2 );
    assert( elRoot.tag = 'html' );

    elBody := THtmlHElement.Create( elRoot.child[1] );
    assert( elBody <> nil );
    assert( elBody.tag = 'body' );

    elEvents := THtmlHElement.Create( elBody.get_element_by_id( 'events' ) );

    elTest := THtmlHElement.Create( elBody.get_element_by_id( 'test' ) );
